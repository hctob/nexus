...
var (
	...
	admin            = flag.Bool("admin", false, "Enable to run shell queries.")
...
func drive(uri, username, password string, cm ChannelPool) {
	...
	for {
		...
		case username := <-cm.get_friends_list:
			...
			for result.Next() {
				ret = ret + fmt.Sprintf("^%s-%s-%s-%s-%s", result.Record().GetByIndex(0).(string), result.Record().GetByIndex(1).(string), result.Record().GetByIndex(2).(string), result.Record().GetByIndex(4).(string), result.Record().GetByIndex(5).(string))
